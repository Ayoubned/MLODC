...
void afficheElement(Music* music) {
	printf("%s by %s (%d)\n", music->name, music->artist, music->year);
}
...
void detruireElement(Music* music) {
    free(music->name);
    free(music->artist);
    free(music->album);
    free(music->genre);
...
bool equalsElement(Element e1, Element e2){
    ...
    // Compare the name, artist, and year for equality
    if (strcmp(music1->name, music2->name) == 0 &&
        strcmp(music1->artist, music2->artist) == 0 &&
        music1->year == music2->year) {
        return true;
    }
    return false;
}
